package com.abdullah.Java6MovieAppMvc.service;


import com.abdullah.Java6MovieAppMvc.dto.request.LoginRequestDto;
...
import com.abdullah.Java6MovieAppMvc.repository.IUserRepository;
...
public class UserService implements IServiceCrud<User> {
    ...
    public User register2(UserRegisterRequestDto dto) {
        if (userRepository.existsByEmail(dto.getEmail())){
            throw new RuntimeException("Email zaten kayıtlıdır");
        }
        if (!dto.getPassword().equals(dto.getRePassword())){
            throw new RuntimeException("girdiginiz sifreler uyuşmamaktadır");
        }
